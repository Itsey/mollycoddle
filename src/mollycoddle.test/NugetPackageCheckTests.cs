...
#pragma warning restore CS8625
    ...
    public void ParseNugetString_Works(string packageString, string packageName, string? mustBeAtLeast, string? mustBeAtMost, PackageVersionMatchType mt) {
        ...
        if (mustBeAtLeast == null) {
            sut.LowVersionNumber.ShouldBeNull();
        } else {
            sut.LowVersionNumber.ShouldNotBeNull();
            sut.LowVersionNumber?.ToString().ShouldBeEquivalentTo(mustBeAtLeast);
        }
        if (mustBeAtMost == null) {
            sut.LowVersionNumber.ShouldBeNull();
        } else {
            sut.HighVersionNumber.ShouldNotBeNull();
            sut.HighVersionNumber?.ToString().ShouldBeEquivalentTo(mustBeAtMost);
        }

        sut.PackageName.ShouldBeEquivalentTo(packageName);
        sut.VersionMatchType.ShouldBe(mt);
    }
}